...
class StateAggregationEncoder(nn.Module):
    ...
    def forward(self, state, value):
        ...
        x = self.fc1(torch.concat([state, torch.scalar_tensor(value)], dim=-1))
...
class StateAggregationAutoEncoder(nn.Module):
    ...
    def save_encoder(self):
        torch.save(self.encoder, "models/state_aggregation_encoder.pt")

    def save_decoder(self):
        torch.save(self.decoder, "models/state_aggregation_decoder.pt")

    def save_autoencoder(self):
        torch.save(self, "models/state_aggregation_autoencoder.pt")
...
def train_state_aggregation_encoder(env_id, env_seed):
    ...
    dataset = [env.observation_space.sample() for i in range(NUM_SAMPLES)]
    loss_fn = nn.MSELoss()

    for i in range(EPOCHS):

        sample = torch.tensor(random.sample(dataset, BATCH_SIZE), dtype=torch.float32)
        autoencoder_out = autoencoder(sample)
...
"""
    ...
    train_state_aggregation_encoder(args.env_id, args.seed)
    # assert isinstance(
